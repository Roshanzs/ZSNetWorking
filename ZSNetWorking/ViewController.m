...
@property(nonatomic,strong)ZSNetworking *tool;
@property(nonatomic,strong)ZSNetworking *tool2;
...
- (void)viewDidLoad {
    ...
    self.tool = z;
//    self.tool.loadFinish = ^{
//        NSLog(@"下载完毕");
//    };
    [z DownloadWithURLString:MP4_URL_String setProgressValue:^(float progress) {
        NSLog(@"%f",progress);
    } downloadfinish:^{
        NSLog(@"下载完毕");

    }];
//    [z DownloadWithURLString:MP4_URL_String2 setProgressValue:^(float progressValue) {
